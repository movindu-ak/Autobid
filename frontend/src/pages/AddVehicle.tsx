...
export default function AddVehicle() {
  ...
  const [formData, setFormData] = useState({
    ...
    engineCapacity: '',
    previousOwners: '',
    insuranceClaims: 'Never' as 'Never' | 'Once' | 'Twice' | '3 times' | '4 times' | '5+ times',
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 py-8">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-lg p-8 space-y-6">
          ...
          <div>
            <label htmlFor="insuranceClaims" className="block text-sm font-medium text-gray-700 mb-2">
              Insurance Claims History
            </label>
            <select
              id="insuranceClaims"
              value={formData.insuranceClaims}
              onChange={(e) => setFormData({ ...formData, insuranceClaims: e.target.value as 'Never' | 'Once' | 'Twice' | '3 times' | '4 times' | '5+ times' })}
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              required
            >
            ...
            <p className="text-xs text-gray-500 mt-1">
              How many times has insurance been claimed for this vehicle?
            </p>
          </div>
